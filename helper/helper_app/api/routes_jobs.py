...
from helper_app.api import routes_aws_vms, routes_azure_vms, routes_gcp_vms, routes_hyperv_vms, routes_olvm_vms
from helper_app.api.routes_vms import inspect
from helper_app.auth import (
    require_aws_session,
    require_azure_session,
    require_gcp_session,
    require_hyperv_session,
...
@router.post("/hyperv", response_model=Job, status_code=status.HTTP_202_ACCEPTED)
async def create_hyperv_job(body: CreateHypervJobRequest, request: Request,
                            session: UserSession = Depends(require_hyperv_session)):
    """Migrate a Hyper-V VM: it is shut down, then its VHD/VHDX files are read onto OCI volumes."""
    st = request.app.state
    details = await asyncio.to_thread(routes_hyperv_vms.inspect_details, session, body.vm_id)
    inspection = routes_hyperv_vms.inspect(session, body.vm_id, details)
    if not inspection.can_export:
        raise HTTPException(status.HTTP_400_BAD_REQUEST, "; ".join(inspection.problems))
    if inspection.needs_power_off and not body.power_off_source:
        raise HTTPException(status.HTTP_400_BAD_REQUEST,
                            f"{inspection.vm.name} is powered on; confirm that it may be shut down for the "
                            "migration (power_off_source), or power it off in Hyper-V Manager first")
    await _check_guest_os(st, inspection, body.target, "Hyper-V")
    active = st.store.active_for_vm(inspection.vm.moid)
    if active is not None:
        raise HTTPException(status.HTTP_409_CONFLICT, f"job {active.id} for this VM is still {active.phase.value}")
    now = utcnow()
    hyperv = session.hyperv
    job = Job(
        id=uuid.uuid4().hex,
        kind="hyperv",
        vm=inspection.vm,
        hyperv=HypervSourceInfo(host=hyperv.hostname, disks=details.chains),
        target=body.target,
        power_off_source=inspection.needs_power_off,
        phase=JobPhase.QUEUED,
        message="Queued" + (" (the VM is shut down right before the disk export)" if inspection.needs_power_off
